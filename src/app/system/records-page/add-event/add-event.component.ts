    ...
    this.billService.getBills().subscribe((bills: Bill[]) => {
      ...
      if(type === 'outcome') {
        if(amount > bill.value) {
          this.showMessage(`На счету не достаточно средств. Вам не хватает ${bill.value - amount}`);
          return;
      ...
      this.billService.updateBill(bill.id,{value, currency: bill.currency})
